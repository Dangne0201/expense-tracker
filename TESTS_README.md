...
1) Unit tests (fast, do not touch DB)
   - Powershell: .\run-unit-tests.ps1
   - Command: dotnet test src/ExpenseTracker.Tests -c Debug

2) Integration tests (need SQL Server)
   - Start DB/init: .\setup-all.ps1 -saPassword 'Your_password123' -RunApp:$false
   - Or run helper: .\run-integration-tests.ps1 -saPassword 'Your_password123'
   - Command: dotnet test src/ExpenseTracker.Tests -c Debug
...
Notes & tips
 ...
 - For integration tests prefer to run DB in Docker (setup-all.ps1) and run tests inside a transaction and roll it back.
