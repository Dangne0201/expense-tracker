...
Usage:
  PowerShell -ExecutionPolicy Bypass -File .\run-ui-tests.ps1 -Configuration Debug
#>
param(
...
dotnet build "src/ExpenseTracker.WinForms/ExpenseTracker.WinForms.csproj" -c $Configuration || throw "WinForms build failed"

# Build and run UI tests
dotnet test "src/ExpenseTracker.UiTests" -c $Configuration --no-build --verbosity minimal
